...
export default {
  ...
  async mounted() {
    //get blog id from params and use it to get the blog
    this.routeID = this.$route.params.blogid;
    this.currentBlog = await this.$store.state.blogPosts.filter(post => post.blogID === this.routeID);

    this.$store.commit('setBlogState', this.currentBlog[0]); //set the blog
  },
      ...
      if (this.blogTitle.length !== 0 && this.blogHTML.length !== 0) {
        ...
        if (this.file) {
          ...
          docRef.put(this.file).on('state_changed', (snapshot) => {
            ...
            await this.$store.dispatch('updatePost', this.routeID); //update post list
        ...
        //if no file, just update html and title
        this.loading = true;
        ...
        await this.$store.dispatch('updatePost', this.routeID); //update list of posts in store

        this.loading = false;

        this.$router.push({name: 'ViewBlog', params: {blogid: database.id}}); //reroute to blog
